...
> **Warning:** Make sure to have a backup of all important files in a separate location like on a flash drive or in OneDrive before procedding any further. Using a UNIX Terminal on any computer can change the computer's file system and data.
...
1) Click on the ``Spotlight Search`` icon located in the menu bar at the top right of the screen. 
   
   [insert image here]

   If you do not have the icon in your menu, you can alternatively use the ``Command + Space`` keyboard shortcut.

2) Type `Terminal` or `terminal` into the ``Spotlight Search`` popup
